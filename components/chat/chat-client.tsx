...
import type { Contact, ChatMessageDTO, Presence } from "@/lib/chat/types"
...
function Avatar({ username, online = false }: { username: string; online?: boolean }) {
  return <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-jci-blue text-sm font-semibold text-primary-foreground">
    ...
    {online && <span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-card bg-jci-teal" />}
  </div>
}
...
export function ChatClient() {
  ...
  async function changePassword(e: React.FormEvent) { e.preventDefault(); setPasswordError(""); try { await json("/api/chat/password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password }) }); setPassword(""); setPasswordOpen(false) } catch (error) { setPasswordError(error instanceof Error ? error.message : "Unable to update password") } }
