...
export function ChatClient() {
  ...
  // Forces a jump to the newest message right after switching conversations.
  ...
  useEffect(() => {
    ...
    if (!container) return
    // Only auto-scroll if the user is already near the bottom, so polling
    // refreshes don't yank them down while they read older messages.
  ...
  async function sendMessage() {
    ...
    setSending(true)
    try { const data = await json("/api/chat/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ recipient: selected, body: draft.trim() }) }); forceScrollRef.current = true; setMessages((current) => [...current, data.message]); setDraft("") } finally { setSending(false) }
  }
